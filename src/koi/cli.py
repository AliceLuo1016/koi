...
_DEFAULT_AGENTS_MD = (
    ...
    "1. Read `.koi/MEMORY.md` for long-term "
    "context.\n"
    "2. Read today and yesterday's daily logs "
    "(`memory/YYYY-MM-DD.md`).\n"
    "3. Check `.koi/alerts/` for any pending "
    "alerts.\n"
    "4. Then proceed with the user's request.\n\n"
    "## Core Behavior\n\n"
    "- Learn from execution history and prefer the "
    "shortest safe path to achieve the goal.\n"
    "- When you spot repeated steps or errors, "
    "update the relevant skill(s) or memory with "
    "the more efficient path.\n"
    "- When a faster workflow emerges, apply it "
    "directly in future runs.\n\n"
    "## Memory\n\n"
    "You wake up fresh each session. These files "
    "are your continuity:\n\n"
    "- **Daily logs:** `memory/YYYY-MM-DD.md` — "
    "raw notes from each session. Append-only.\n"
    "- **Long-term:** `MEMORY.md` — curated "
    "memory. Preferences, key facts, project-wide "
    "patterns.\n\n"
    "**Where to store learnings:**\n\n"
    "- **Skill-specific** → Update the skill's "
    "`SKILL.md` directly.\n"
    "- **General** → Write to `MEMORY.md`.\n\n"
    "When something goes wrong, document the fix "
    "so future-you can skip the pain.\n\n"
    "## Output & Alerts\n\n"
    "- In interactive sessions, output results "
    "directly in the terminal.\n"
    "- Reserve `create_alert` for cron jobs "
    "(non-interactive runs).\n\n"
    "## Running Scripts\n\n"
    "Use `uv run python` to run Python scripts — "
    "this automatically activates the project's "
    "virtual environment.\n\n"
    "## Skills\n\n"
    "All skills live in `.koi/skills/`. Each skill "
    "is a directory containing a `SKILL.md` file. "
    "Use `read_skill` with the directory name to "
    "load a skill.\n"
)
